...
fore_expxmth = forecast(data_ts_expsmth, h = 12)
plot(fore_expxmth)
fore_expxmth
```
...
fore_holtexpsmth = forecast(data_ts_holt_expsmth, h = 12)
plot(fore_holtexpsmth)
fore_holtexpsmth
```
...
plot(fit$x)
fore_arimamine = forecast(fit, h = 12)
plot(fore_arimamine)
fore_arimamine
...
plot(fit2$x)
fore_arimaauto = forecast(fit2, h = 12)
plot(fore_arimaauto)
fore_arimaauto

#prophet_df_ts2 = prophet(prophet_df2)
#fitting4 = make_future_dataframe(prophet_df_ts2, periods = 365)
...
fitting2 = make_future_dataframe(prophet_df_ts, periods = 365)
prediction = predict(prophet_df_ts, fitting2)
plot(prophet_df_ts, prediction)


#These take prediction and make it into a prediction table by MONTH instead of accounting for every year
prophet_sum = data.frame(c(1:12))
prophet_sum$trend = 0

prophet_pred_2022 = data.frame(subset(prediction, year(prediction$ds) == 2022))
prophet_pred_2022 = prophet_pred_2022[,1:2]
prophet_pred_2022$ds = month(prophet_pred_2022$ds)
i = 1

while(length(prophet_pred_2022$ds) >= i){
  curmonth = prophet_pred_2022$ds[i]
  prophet_sum$trend[curmonth] = prophet_sum$trend[curmonth] + prophet_pred_2022$trend[i]
  i = i + 1
}
